...
@property (nonatomic, strong) NSIndexPath *srcIndexPath;
@end
...
- (void)jx_gestureBegan:(UILongPressGestureRecognizer *)gesture
{
    ...
    self.srcIndexPath = selectedIndexPath;
...
- (void)jx_gestureChanged:(UILongPressGestureRecognizer *)gesture
{
    ...
    if (currentIndexPath &&
        ![self.selectedIndexPath isEqual:currentIndexPath] &&
        self.selectedIndexPath.section != currentIndexPath.section) {
        //交换数据源和cell
...
//    _tempView.center = CGPointMake(_tempView.center.x, point.y);
    ...
    NSLog(@"tmpView.frame is %@",NSStringFromCGRect(_tempView.frame));
}
...
- (void)jx_gestureEndedOrCancelled:(UILongPressGestureRecognizer *)gesture
{
    ...
    if (self.srcIndexPath.section != self.selectedIndexPath.section &&
        self.selectedIndexPath.section == 0 &&
        self.srcIndexPath.section == 1) {
        [_tempDataSource removeObjectAtIndex:self.srcIndexPath.row];
    }
    //返回交换后的数据源
    ...
    if (self.srcIndexPath.section != self.selectedIndexPath.section &&
        self.selectedIndexPath.section == 0 &&
        self.srcIndexPath.section == 1) {
        cell = [self cellForRowAtIndexPath:self.selectedIndexPath];
    } else {
        cell = [self cellForRowAtIndexPath:self.srcIndexPath];
    }
    [UIView animateWithDuration:kJXMovableCellAnimationTime animations:^{
        _tempView.frame = cell.frame;
        ...
        if (self.srcIndexPath.section != self.selectedIndexPath.section &&
            self.selectedIndexPath.section == 0 &&
            self.srcIndexPath.section == 1) {
            [self deleteRowsAtIndexPaths:@[self.srcIndexPath] withRowAnimation:UITableViewRowAnimationNone];
...
- (void)jx_processEdgeScroll
{
    ...
    if (touchPoint.y < _edgeScrollRange) {
        if (self.contentOffset.y <= 0 - dHeight) {
            return;
        }else {
            if (self.contentOffset.y - 1 < 0 - dHeight) {
                return;
            }
